...
from psystack.cli.wizard import discovery, questions, review
from psystack.cli.wizard.discovery import WizardDiscoveryError
from psystack.cli.wizard.models import ChangeType, InitAnswers, WeightInfo
...
def write_workspace_files(workspace: Path, answers: InitAnswers) -> list[Path]:
    """Write psystack.toml and manifests. Returns list of written paths. No console output."""
    ...
    config_path = workspace / "psystack.toml"
...
def _write_outputs(workspace: Path, answers: InitAnswers) -> None:
    ...
    typer.echo(f"\nDone! Next: psystack run --workspace {workspace}")
